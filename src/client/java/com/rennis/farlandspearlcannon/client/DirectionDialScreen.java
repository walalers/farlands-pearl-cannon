...
public class DirectionDialScreen extends Screen {
    ...
    public DirectionDialScreen(BlockPos corePos, int currentIndex) {
        super(Minecraft.getInstance(), Minecraft.getInstance().font, Component.translatable("gui.farlands_pearl_cannon.dial.title"));
    ...
    protected void init() {
        ...
        for (FarlandsTarget target : FarlandsTarget.values()) {
            ...
            Component label = current
                    ? target.label().withStyle(ChatFormatting.GREEN, ChatFormatting.BOLD)
                    : target.label();
    ...
    public void extractRenderState(GuiGraphicsExtractor graphics, int mouseX, int mouseY, float delta) {
        ...
        Component current = Component.translatable("gui.farlands_pearl_cannon.dial.current", FarlandsTarget.values()[currentIndex].label())
